...
- [ ] 6.1 SearchDialog — 전체화면 검색(제목·메모), 결과 클릭 시 해당 날짜/일정으로 이동
- [ ] 6.2 키보드 단축키 — stepDate 공용 유틸 추출, T/월·주·일·목록/←→/N/'/' 전역 단축키 (입력 중엔 무시)
- [ ] 6.3 JSON 내보내기/가져오기 — 이벤트·카테고리 백업 다운로드/복원
- [ ] 6.4 모바일 대응 — EventEditor 바텀시트(작은 화면), 좌우 스와이프로 기간 이동, 하단 고정 + 버튼
- [ ] 6.5 ponytail 점검
