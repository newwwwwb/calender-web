...
- [ ] 20.1 스와이프·스크롤 충돌 — 가로 의도 판정 후에만 드래그, 커밋 기준 강화, 스와이프 후 탭 차단, 스크롤 컨테이너 pan-y
- [ ] 20.2 시간 그리드 현재 시각 근처에서 시작 + 현재 시각 선 + 하단 여백
- [ ] 20.3 viewport-fit=cover·theme-color, 100dvh, 세이프 에어리어
- [ ] 20.4 전역 터치 위생 — 탭 하이라이트, 입력 16px, hover:hover, :active, 11px 이상 글자
- [ ] 20.5 터치 영역 44px + 터치에서 보이는 삭제 버튼 + 할 일 추가 폼
- [ ] 20.6 시트 구조 — 고정 헤더 + 스크롤 본문, 스크롤 잠금, 바운스 없는 진입
- [ ] 20.7 편집 시트 상단 바 [취소] 제목 [저장]
- [ ] 20.8 모바일 헤더 2줄(큰 월 제목 → 날짜 선택, 세그먼트) + 설정에 계정
- [ ] 20.9 모바일 월 보기 — 점 + 선택일 일정 목록
- [ ] 20.10 모바일 주 블록 iOS식(제목 줄바꿈)
- [ ] 20.11 FAB SVG 아이콘
- [ ] 20.12 혹독한 보스 검증 루프(서브에이전트 6개, 모바일 집중)
