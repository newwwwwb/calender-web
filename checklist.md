...
## 21단계: 바탕화면 위젯 (Edge 앱 창 + AutoHotkey, 웹과 양방향 자동 갱신)
- [x] 21.1 useCalendar 주기 갱신 — 60초 + 포커스/탭 복귀 시 reload (웹·위젯 공통, 뒤늦은 응답이 최신 데이터를 덮지 않게 방지) + 테스트
- [ ] 21.2 위젯 모드 — `?widget=1`일 때만 배경 토큰을 키 색으로 덮어써 배경만 투명 (웹 디자인 불변, 팝업은 불투명)
- [ ] 21.3 desktop-widget/calendar-widget.ahk — Edge --app 전용 프로필 실행, 위치/크기, 맨 아래 고정, 색 키 투명, 부팅 직후 재시도 + 시작프로그램 등록
- [ ] 21.4 스파이크 검증 — Win+D 후 표시, 색 키 적용·글자 테두리, 로그인 유지, 양방향 반영, 재부팅 자동 실행 (안 되면 창 전체 반투명으로 되돌림)
- [ ] 21.5 ponytail 점검 + 결과를 context-notes에 기록
